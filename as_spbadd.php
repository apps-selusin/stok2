...
$as_spb_add = NULL; // Initialize page object first

class cas_spb_add extends cas_spb {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["spbID"] != "") {
				...
				$this->setKey("spbID", $this->spbID->CurrentValue); // Set up key
			} else {
				$this->setKey("spbID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("as_spblist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "as_spbview.php")
...
// Create page object
if (!isset($as_spb_add)) $as_spb_add = new cas_spb_add();

// Page init
$as_spb_add->Page_Init();

// Page main
$as_spb_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$as_spb_add->Page_Render();
?>
...
var CurrentForm = fas_spbadd = new ew_Form("fas_spbadd", "add");

// Validate form
fas_spbadd.Validate = function() {
...
fas_spbadd.Form_CustomValidate = 
...
fas_spbadd.ValidateRequired = true;
<?php } else { ?>
fas_spbadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $as_spb_add->ShowPageHeader(); ?>
<?php
$as_spb_add->ShowMessage();
?>
<form name="fas_spbadd" id="fas_spbadd" class="<?php echo $as_spb_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($as_spb_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $as_spb_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $as_spb_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
