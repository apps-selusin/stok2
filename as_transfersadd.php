...
$as_transfers_add = NULL; // Initialize page object first

class cas_transfers_add extends cas_transfers {
	...
	// Page object name
	var $PageObjName = 'as_transfers_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["transferID"] != "") {
				...
				$this->setKey("transferID", $this->transferID->CurrentValue); // Set up key
			} else {
				$this->setKey("transferID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("as_transferslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "as_transfersview.php")
	...
	function LoadDefaultValues() {
		...
		$this->trxDate->CurrentValue = NULL;
		$this->trxDate->OldValue = $this->trxDate->CurrentValue;
		$this->note->CurrentValue = NULL;
		$this->note->OldValue = $this->note->CurrentValue;
		$this->transferFrom->CurrentValue = NULL;
		$this->transferFrom->OldValue = $this->transferFrom->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// trxDate
		$this->trxDate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->trxDate->CurrentValue, 0), ew_CurrentDate(), FALSE);

		// note
		$this->note->SetDbValueDef($rsnew, $this->note->CurrentValue, "", FALSE);

		// transferFrom
		$this->transferFrom->SetDbValueDef($rsnew, $this->transferFrom->CurrentValue, 0, FALSE);

		// transferTo
		$this->transferTo->SetDbValueDef($rsnew, $this->transferTo->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($as_transfers_add)) $as_transfers_add = new cas_transfers_add();

// Page init
$as_transfers_add->Page_Init();

// Page main
$as_transfers_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$as_transfers_add->Page_Render();
?>
...
var CurrentForm = fas_transfersadd = new ew_Form("fas_transfersadd", "add");

// Validate form
fas_transfersadd.Validate = function() {
...
fas_transfersadd.Form_CustomValidate = 
...
fas_transfersadd.ValidateRequired = true;
<?php } else { ?>
fas_transfersadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$as_transfers_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $as_transfers_add->ShowPageHeader(); ?>
<?php
$as_transfers_add->ShowMessage();
?>
<form name="fas_transfersadd" id="fas_transfersadd" class="<?php echo $as_transfers_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($as_transfers_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $as_transfers_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $as_transfers_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fas_transfersadd.Init();
</script>
<?php
$as_transfers_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$as_transfers_add->Page_Terminate();
?>
