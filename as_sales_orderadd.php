...
$as_sales_order_add = NULL; // Initialize page object first

class cas_sales_order_add extends cas_sales_order {
	...
	// Page object name
	var $PageObjName = 'as_sales_order_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("as_sales_orderlist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "as_sales_orderview.php")
...
// Create page object
if (!isset($as_sales_order_add)) $as_sales_order_add = new cas_sales_order_add();

// Page init
$as_sales_order_add->Page_Init();

// Page main
$as_sales_order_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$as_sales_order_add->Page_Render();
?>
...
var CurrentForm = fas_sales_orderadd = new ew_Form("fas_sales_orderadd", "add");

// Validate form
fas_sales_orderadd.Validate = function() {
...
fas_sales_orderadd.Form_CustomValidate = 
...
fas_sales_orderadd.ValidateRequired = true;
<?php } else { ?>
fas_sales_orderadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$as_sales_order_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $as_sales_order_add->ShowPageHeader(); ?>
<?php
$as_sales_order_add->ShowMessage();
?>
<form name="fas_sales_orderadd" id="fas_sales_orderadd" class="<?php echo $as_sales_order_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($as_sales_order_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $as_sales_order_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $as_sales_order_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fas_sales_orderadd.Init();
</script>
<?php
$as_sales_order_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$as_sales_order_add->Page_Terminate();
?>
