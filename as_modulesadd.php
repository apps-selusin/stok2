...
$as_modules_add = NULL; // Initialize page object first

class cas_modules_add extends cas_modules {
	...
	// Page object name
	var $PageObjName = 'as_modules_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["modulID"] != "") {
				...
				$this->setKey("modulID", $this->modulID->CurrentValue); // Set up key
			} else {
				$this->setKey("modulID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("as_moduleslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "as_modulesview.php")
	...
	function LoadDefaultValues() {
		$this->modulName->CurrentValue = NULL;
		$this->modulName->OldValue = $this->modulName->CurrentValue;
		$this->authorize->CurrentValue = NULL;
		$this->authorize->OldValue = $this->authorize->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// modulName
		$this->modulName->SetDbValueDef($rsnew, $this->modulName->CurrentValue, "", FALSE);

		// authorize
		$this->authorize->SetDbValueDef($rsnew, $this->authorize->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($as_modules_add)) $as_modules_add = new cas_modules_add();

// Page init
$as_modules_add->Page_Init();

// Page main
$as_modules_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$as_modules_add->Page_Render();
?>
...
var CurrentForm = fas_modulesadd = new ew_Form("fas_modulesadd", "add");

// Validate form
fas_modulesadd.Validate = function() {
...
fas_modulesadd.Form_CustomValidate = 
...
fas_modulesadd.ValidateRequired = true;
<?php } else { ?>
fas_modulesadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $as_modules_add->ShowPageHeader(); ?>
<?php
$as_modules_add->ShowMessage();
?>
<form name="fas_modulesadd" id="fas_modulesadd" class="<?php echo $as_modules_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($as_modules_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $as_modules_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $as_modules_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
