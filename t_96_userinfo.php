...
//
// Table class for t_96_user
//
class ct_96_user extends cTable {
	var $_userid;
	var $username;
	var $password;
	var $userlevel;
	...
	function __construct() {
		...
		$this->TableVar = 't_96_user';
		$this->TableName = 't_96_user';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_96_user`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// userid
		$this->_userid = new cField('t_96_user', 't_96_user', 'x__userid', 'userid', '`userid`', '`userid`', 3, -1, FALSE, '`userid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->_userid->Sortable = TRUE; // Allow sort
		$this->_userid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userid'] = &$this->_userid;

		// username
		$this->username = new cField('t_96_user', 't_96_user', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->username->Sortable = TRUE; // Allow sort
		$this->fields['username'] = &$this->username;

		// password
		$this->password = new cField('t_96_user', 't_96_user', 'x_password', 'password', '`password`', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// userlevel
		$this->userlevel = new cField('t_96_user', 't_96_user', 'x_userlevel', 'userlevel', '`userlevel`', '`userlevel`', 3, -1, FALSE, '`userlevel`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// activated
		$this->activated = new cField('t_96_user', 't_96_user', 'x_activated', 'activated', '`activated`', '`activated`', 16, -1, FALSE, '`activated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_96_user`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_96_userview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_96_userview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_96_useradd.php?" . $this->UrlParm($parm);
		else
			$url = "t_96_useradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_96_useredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_96_useradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_96_userdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		...
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `t_96_user`";
