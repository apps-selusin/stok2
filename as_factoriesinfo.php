...
//
// Table class for as_factories
//
class cas_factories extends cTable {
	var $factoryID;
	var $factoryCode;
	var $factoryName;
	var $factoryType;
	...
	function __construct() {
		...
		$this->TableVar = 'as_factories';
		$this->TableName = 'as_factories';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`as_factories`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// factoryID
		$this->factoryID = new cField('as_factories', 'as_factories', 'x_factoryID', 'factoryID', '`factoryID`', '`factoryID`', 3, -1, FALSE, '`factoryID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// factoryCode
		$this->factoryCode = new cField('as_factories', 'as_factories', 'x_factoryCode', 'factoryCode', '`factoryCode`', '`factoryCode`', 200, -1, FALSE, '`factoryCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->factoryCode->Sortable = TRUE; // Allow sort
		$this->fields['factoryCode'] = &$this->factoryCode;

		// factoryName
		$this->factoryName = new cField('as_factories', 'as_factories', 'x_factoryName', 'factoryName', '`factoryName`', '`factoryName`', 200, -1, FALSE, '`factoryName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->factoryName->Sortable = TRUE; // Allow sort
		$this->fields['factoryName'] = &$this->factoryName;

		// factoryType
		$this->factoryType = new cField('as_factories', 'as_factories', 'x_factoryType', 'factoryType', '`factoryType`', '`factoryType`', 200, -1, FALSE, '`factoryType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->factoryType->Sortable = TRUE; // Allow sort
		$this->fields['factoryType'] = &$this->factoryType;

		// status
		$this->status = new cField('as_factories', 'as_factories', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->status->Sortable = TRUE; // Allow sort
		$this->fields['status'] = &$this->status;

		// note
		$this->note = new cField('as_factories', 'as_factories', 'x_note', 'note', '`note`', '`note`', 201, -1, FALSE, '`note`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->note->Sortable = TRUE; // Allow sort
		$this->fields['note'] = &$this->note;

		// createdDate
		$this->createdDate = new cField('as_factories', 'as_factories', 'x_createdDate', 'createdDate', '`createdDate`', ew_CastDateFieldForLike('`createdDate`', 0, "DB"), 135, 0, FALSE, '`createdDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// createdUserID
		$this->createdUserID = new cField('as_factories', 'as_factories', 'x_createdUserID', 'createdUserID', '`createdUserID`', '`createdUserID`', 3, -1, FALSE, '`createdUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedDate
		$this->modifiedDate = new cField('as_factories', 'as_factories', 'x_modifiedDate', 'modifiedDate', '`modifiedDate`', ew_CastDateFieldForLike('`modifiedDate`', 0, "DB"), 135, 0, FALSE, '`modifiedDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedUserID
		$this->modifiedUserID = new cField('as_factories', 'as_factories', 'x_modifiedUserID', 'modifiedUserID', '`modifiedUserID`', '`modifiedUserID`', 3, -1, FALSE, '`modifiedUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`as_factories`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->factoryID->setDbValue($conn->Insert_ID());
			$rs['factoryID'] = $this->factoryID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('factoryID', $rs))
				ew_AddFilter($where, ew_QuotedName('factoryID', $this->DBID) . '=' . ew_QuotedValue($rs['factoryID'], $this->factoryID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->factoryID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@factoryID@", ew_AdjustSql($this->factoryID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("as_factoriesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("as_factoriesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "as_factoriesadd.php?" . $this->UrlParm($parm);
		else
			$url = "as_factoriesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("as_factoriesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("as_factoriesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("as_factoriesdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "factoryID:" . ew_VarToJson($this->factoryID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->factoryID->CurrentValue)) {
			$sUrl .= "factoryID=" . urlencode($this->factoryID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["factoryID"]))
				$arKeys[] = ew_StripSlashes($_POST["factoryID"]);
			elseif (isset($_GET["factoryID"]))
				$arKeys[] = ew_StripSlashes($_GET["factoryID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->factoryID->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->factoryCode->EditValue = $this->factoryCode->CurrentValue;
		...
		$this->factoryType->EditValue = $this->factoryType->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->factoryCode->Exportable) $Doc->ExportCaption($this->factoryCode);
					if ($this->factoryName->Exportable) $Doc->ExportCaption($this->factoryName);
					if ($this->factoryType->Exportable) $Doc->ExportCaption($this->factoryType);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->factoryCode->Exportable) $Doc->ExportField($this->factoryCode);
						if ($this->factoryName->Exportable) $Doc->ExportField($this->factoryName);
						if ($this->factoryType->Exportable) $Doc->ExportField($this->factoryType);
