...
$as_detail_do_add = NULL; // Initialize page object first

class cas_detail_do_add extends cas_detail_do {
	...
	// Page object name
	var $PageObjName = 'as_detail_do_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("as_detail_dolist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "as_detail_doview.php")
	...
	function LoadDefaultValues() {
		...
		$this->deliveredQty->CurrentValue = NULL;
		$this->deliveredQty->OldValue = $this->deliveredQty->CurrentValue;
		$this->deliveredStatus->CurrentValue = NULL;
		$this->deliveredStatus->OldValue = $this->deliveredStatus->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// deliveredQty
		$this->deliveredQty->SetDbValueDef($rsnew, $this->deliveredQty->CurrentValue, 0, FALSE);

		// deliveredStatus
		$this->deliveredStatus->SetDbValueDef($rsnew, $this->deliveredStatus->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($as_detail_do_add)) $as_detail_do_add = new cas_detail_do_add();

// Page init
$as_detail_do_add->Page_Init();

// Page main
$as_detail_do_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$as_detail_do_add->Page_Render();
?>
...
var CurrentForm = fas_detail_doadd = new ew_Form("fas_detail_doadd", "add");

// Validate form
fas_detail_doadd.Validate = function() {
...
fas_detail_doadd.Form_CustomValidate = 
...
fas_detail_doadd.ValidateRequired = true;
<?php } else { ?>
fas_detail_doadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$as_detail_do_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $as_detail_do_add->ShowPageHeader(); ?>
<?php
$as_detail_do_add->ShowMessage();
?>
<form name="fas_detail_doadd" id="fas_detail_doadd" class="<?php echo $as_detail_do_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($as_detail_do_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $as_detail_do_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $as_detail_do_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fas_detail_doadd.Init();
</script>
<?php
$as_detail_do_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$as_detail_do_add->Page_Terminate();
?>
