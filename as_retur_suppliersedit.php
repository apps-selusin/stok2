...
$as_retur_suppliers_edit = NULL; // Initialize page object first

class cas_retur_suppliers_edit extends cas_retur_suppliers {
	...
	// Page object name
	var $PageObjName = 'as_retur_suppliers_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("as_retur_supplierslist.php"); // Return to list page
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "as_retur_supplierslist.php")
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// returDate
			$this->returDate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->returDate->CurrentValue, 0), ew_CurrentDate(), $this->returDate->ReadOnly);
			...
			// returType
			$this->returType->SetDbValueDef($rsnew, $this->returType->CurrentValue, "", $this->returType->ReadOnly);
...
// Create page object
if (!isset($as_retur_suppliers_edit)) $as_retur_suppliers_edit = new cas_retur_suppliers_edit();

// Page init
$as_retur_suppliers_edit->Page_Init();

// Page main
$as_retur_suppliers_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$as_retur_suppliers_edit->Page_Render();
?>
...
var CurrentForm = fas_retur_suppliersedit = new ew_Form("fas_retur_suppliersedit", "edit");

// Validate form
fas_retur_suppliersedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->returNo->FldCaption(), $as_retur_suppliers->returNo->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_returDate");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->returDate->FldCaption(), $as_retur_suppliers->returDate->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_returDate");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->returDate->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_invoiceNo");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->invoiceNo->FldCaption(), $as_retur_suppliers->invoiceNo->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_supplierID");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->supplierID->FldCaption(), $as_retur_suppliers->supplierID->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_supplierID");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->supplierID->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_supplierName");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->supplierName->FldCaption(), $as_retur_suppliers->supplierName->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_supplierAddress");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->supplierAddress->FldCaption(), $as_retur_suppliers->supplierAddress->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_returType");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->returType->FldCaption(), $as_retur_suppliers->returType->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_subtotal");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->subtotal->FldCaption(), $as_retur_suppliers->subtotal->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_subtotal");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->subtotal->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_ppnType");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->ppnType->FldCaption(), $as_retur_suppliers->ppnType->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ppnType");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->ppnType->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_ppn");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->ppn->FldCaption(), $as_retur_suppliers->ppn->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ppn");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->ppn->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_grandtotal");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->grandtotal->FldCaption(), $as_retur_suppliers->grandtotal->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_grandtotal");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->grandtotal->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_staffID");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->staffID->FldCaption(), $as_retur_suppliers->staffID->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_staffID");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->staffID->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_staffName");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->staffName->FldCaption(), $as_retur_suppliers->staffName->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ref");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->ref->FldCaption(), $as_retur_suppliers->ref->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_note");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->note->FldCaption(), $as_retur_suppliers->note->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_createdDate");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->createdDate->FldCaption(), $as_retur_suppliers->createdDate->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_createdDate");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->createdDate->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_createdUserID");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->createdUserID->FldCaption(), $as_retur_suppliers->createdUserID->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_createdUserID");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->createdUserID->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_modifiedDate");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->modifiedDate->FldCaption(), $as_retur_suppliers->modifiedDate->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_modifiedDate");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->modifiedDate->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_modifiedUserID");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $as_retur_suppliers->modifiedUserID->FldCaption(), $as_retur_suppliers->modifiedUserID->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_modifiedUserID");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($as_retur_suppliers->modifiedUserID->FldErrMsg()) ?>");
...
fas_retur_suppliersedit.Form_CustomValidate = 
...
fas_retur_suppliersedit.ValidateRequired = true;
<?php } else { ?>
fas_retur_suppliersedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$as_retur_suppliers_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $as_retur_suppliers_edit->ShowPageHeader(); ?>
<?php
$as_retur_suppliers_edit->ShowMessage();
?>
<?php if (!$as_retur_suppliers_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($as_retur_suppliers_edit->Pager)) $as_retur_suppliers_edit->Pager = new cPrevNextPager($as_retur_suppliers_edit->StartRec, $as_retur_suppliers_edit->DisplayRecs, $as_retur_suppliers_edit->TotalRecs) ?>
<?php if ($as_retur_suppliers_edit->Pager->RecordCount > 0 && $as_retur_suppliers_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($as_retur_suppliers_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $as_retur_suppliers_edit->PageUrl() ?>start=<?php echo $as_retur_suppliers_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($as_retur_suppliers_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $as_retur_suppliers_edit->PageUrl() ?>start=<?php echo $as_retur_suppliers_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $as_retur_suppliers_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($as_retur_suppliers_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $as_retur_suppliers_edit->PageUrl() ?>start=<?php echo $as_retur_suppliers_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($as_retur_suppliers_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $as_retur_suppliers_edit->PageUrl() ?>start=<?php echo $as_retur_suppliers_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $as_retur_suppliers_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fas_retur_suppliersedit" id="fas_retur_suppliersedit" class="<?php echo $as_retur_suppliers_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($as_retur_suppliers_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $as_retur_suppliers_edit->Token ?>">
<?php } ?>
...
<?php if ($as_retur_suppliers->returID->Visible) { // returID ?>
	<div id="r_returID" class="form-group">
		<label id="elh_as_retur_suppliers_returID" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->returID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->returID->CellAttributes() ?>>
<span id="el_as_retur_suppliers_returID">
<span<?php echo $as_retur_suppliers->returID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $as_retur_suppliers->returID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="as_retur_suppliers" data-field="x_returID" name="x_returID" id="x_returID" value="<?php echo ew_HtmlEncode($as_retur_suppliers->returID->CurrentValue) ?>">
<?php echo $as_retur_suppliers->returID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->returNo->Visible) { // returNo ?>
	<div id="r_returNo" class="form-group">
		<label id="elh_as_retur_suppliers_returNo" for="x_returNo" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->returNo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->returNo->CellAttributes() ?>>
<span id="el_as_retur_suppliers_returNo">
<input type="text" data-table="as_retur_suppliers" data-field="x_returNo" name="x_returNo" id="x_returNo" size="30" maxlength="20" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->returNo->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->returNo->EditValue ?>"<?php echo $as_retur_suppliers->returNo->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->returNo->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->returDate->Visible) { // returDate ?>
	<div id="r_returDate" class="form-group">
		<label id="elh_as_retur_suppliers_returDate" for="x_returDate" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->returDate->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->returDate->CellAttributes() ?>>
<span id="el_as_retur_suppliers_returDate">
<input type="text" data-table="as_retur_suppliers" data-field="x_returDate" name="x_returDate" id="x_returDate" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->returDate->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->returDate->EditValue ?>"<?php echo $as_retur_suppliers->returDate->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->returDate->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->invoiceNo->Visible) { // invoiceNo ?>
	<div id="r_invoiceNo" class="form-group">
		<label id="elh_as_retur_suppliers_invoiceNo" for="x_invoiceNo" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->invoiceNo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->invoiceNo->CellAttributes() ?>>
<span id="el_as_retur_suppliers_invoiceNo">
<input type="text" data-table="as_retur_suppliers" data-field="x_invoiceNo" name="x_invoiceNo" id="x_invoiceNo" size="30" maxlength="20" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->invoiceNo->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->invoiceNo->EditValue ?>"<?php echo $as_retur_suppliers->invoiceNo->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->invoiceNo->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->supplierID->Visible) { // supplierID ?>
	<div id="r_supplierID" class="form-group">
		<label id="elh_as_retur_suppliers_supplierID" for="x_supplierID" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->supplierID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->supplierID->CellAttributes() ?>>
<span id="el_as_retur_suppliers_supplierID">
<input type="text" data-table="as_retur_suppliers" data-field="x_supplierID" name="x_supplierID" id="x_supplierID" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->supplierID->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->supplierID->EditValue ?>"<?php echo $as_retur_suppliers->supplierID->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->supplierID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->supplierName->Visible) { // supplierName ?>
	<div id="r_supplierName" class="form-group">
		<label id="elh_as_retur_suppliers_supplierName" for="x_supplierName" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->supplierName->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->supplierName->CellAttributes() ?>>
<span id="el_as_retur_suppliers_supplierName">
<input type="text" data-table="as_retur_suppliers" data-field="x_supplierName" name="x_supplierName" id="x_supplierName" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->supplierName->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->supplierName->EditValue ?>"<?php echo $as_retur_suppliers->supplierName->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->supplierName->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->supplierAddress->Visible) { // supplierAddress ?>
	<div id="r_supplierAddress" class="form-group">
		<label id="elh_as_retur_suppliers_supplierAddress" for="x_supplierAddress" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->supplierAddress->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->supplierAddress->CellAttributes() ?>>
<span id="el_as_retur_suppliers_supplierAddress">
<textarea data-table="as_retur_suppliers" data-field="x_supplierAddress" name="x_supplierAddress" id="x_supplierAddress" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->supplierAddress->getPlaceHolder()) ?>"<?php echo $as_retur_suppliers->supplierAddress->EditAttributes() ?>><?php echo $as_retur_suppliers->supplierAddress->EditValue ?></textarea>
</span>
<?php echo $as_retur_suppliers->supplierAddress->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->returType->Visible) { // returType ?>
	<div id="r_returType" class="form-group">
		<label id="elh_as_retur_suppliers_returType" for="x_returType" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->returType->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->returType->CellAttributes() ?>>
<span id="el_as_retur_suppliers_returType">
<input type="text" data-table="as_retur_suppliers" data-field="x_returType" name="x_returType" id="x_returType" size="30" maxlength="1" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->returType->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->returType->EditValue ?>"<?php echo $as_retur_suppliers->returType->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->returType->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->subtotal->Visible) { // subtotal ?>
	<div id="r_subtotal" class="form-group">
		<label id="elh_as_retur_suppliers_subtotal" for="x_subtotal" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->subtotal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->subtotal->CellAttributes() ?>>
<span id="el_as_retur_suppliers_subtotal">
<input type="text" data-table="as_retur_suppliers" data-field="x_subtotal" name="x_subtotal" id="x_subtotal" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->subtotal->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->subtotal->EditValue ?>"<?php echo $as_retur_suppliers->subtotal->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->subtotal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->ppnType->Visible) { // ppnType ?>
	<div id="r_ppnType" class="form-group">
		<label id="elh_as_retur_suppliers_ppnType" for="x_ppnType" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->ppnType->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->ppnType->CellAttributes() ?>>
<span id="el_as_retur_suppliers_ppnType">
<input type="text" data-table="as_retur_suppliers" data-field="x_ppnType" name="x_ppnType" id="x_ppnType" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->ppnType->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->ppnType->EditValue ?>"<?php echo $as_retur_suppliers->ppnType->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->ppnType->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->ppn->Visible) { // ppn ?>
	<div id="r_ppn" class="form-group">
		<label id="elh_as_retur_suppliers_ppn" for="x_ppn" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->ppn->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->ppn->CellAttributes() ?>>
<span id="el_as_retur_suppliers_ppn">
<input type="text" data-table="as_retur_suppliers" data-field="x_ppn" name="x_ppn" id="x_ppn" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->ppn->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->ppn->EditValue ?>"<?php echo $as_retur_suppliers->ppn->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->ppn->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->grandtotal->Visible) { // grandtotal ?>
	<div id="r_grandtotal" class="form-group">
		<label id="elh_as_retur_suppliers_grandtotal" for="x_grandtotal" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->grandtotal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->grandtotal->CellAttributes() ?>>
<span id="el_as_retur_suppliers_grandtotal">
<input type="text" data-table="as_retur_suppliers" data-field="x_grandtotal" name="x_grandtotal" id="x_grandtotal" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->grandtotal->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->grandtotal->EditValue ?>"<?php echo $as_retur_suppliers->grandtotal->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->grandtotal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->staffID->Visible) { // staffID ?>
	<div id="r_staffID" class="form-group">
		<label id="elh_as_retur_suppliers_staffID" for="x_staffID" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->staffID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->staffID->CellAttributes() ?>>
<span id="el_as_retur_suppliers_staffID">
<input type="text" data-table="as_retur_suppliers" data-field="x_staffID" name="x_staffID" id="x_staffID" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->staffID->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->staffID->EditValue ?>"<?php echo $as_retur_suppliers->staffID->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->staffID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->staffName->Visible) { // staffName ?>
	<div id="r_staffName" class="form-group">
		<label id="elh_as_retur_suppliers_staffName" for="x_staffName" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->staffName->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->staffName->CellAttributes() ?>>
<span id="el_as_retur_suppliers_staffName">
<input type="text" data-table="as_retur_suppliers" data-field="x_staffName" name="x_staffName" id="x_staffName" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->staffName->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->staffName->EditValue ?>"<?php echo $as_retur_suppliers->staffName->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->staffName->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->ref->Visible) { // ref ?>
	<div id="r_ref" class="form-group">
		<label id="elh_as_retur_suppliers_ref" for="x_ref" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->ref->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->ref->CellAttributes() ?>>
<span id="el_as_retur_suppliers_ref">
<input type="text" data-table="as_retur_suppliers" data-field="x_ref" name="x_ref" id="x_ref" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->ref->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->ref->EditValue ?>"<?php echo $as_retur_suppliers->ref->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->ref->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->note->Visible) { // note ?>
	<div id="r_note" class="form-group">
		<label id="elh_as_retur_suppliers_note" for="x_note" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->note->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->note->CellAttributes() ?>>
<span id="el_as_retur_suppliers_note">
<textarea data-table="as_retur_suppliers" data-field="x_note" name="x_note" id="x_note" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->note->getPlaceHolder()) ?>"<?php echo $as_retur_suppliers->note->EditAttributes() ?>><?php echo $as_retur_suppliers->note->EditValue ?></textarea>
</span>
<?php echo $as_retur_suppliers->note->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->createdDate->Visible) { // createdDate ?>
	<div id="r_createdDate" class="form-group">
		<label id="elh_as_retur_suppliers_createdDate" for="x_createdDate" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->createdDate->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->createdDate->CellAttributes() ?>>
<span id="el_as_retur_suppliers_createdDate">
<input type="text" data-table="as_retur_suppliers" data-field="x_createdDate" name="x_createdDate" id="x_createdDate" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->createdDate->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->createdDate->EditValue ?>"<?php echo $as_retur_suppliers->createdDate->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->createdDate->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->createdUserID->Visible) { // createdUserID ?>
	<div id="r_createdUserID" class="form-group">
		<label id="elh_as_retur_suppliers_createdUserID" for="x_createdUserID" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->createdUserID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->createdUserID->CellAttributes() ?>>
<span id="el_as_retur_suppliers_createdUserID">
<input type="text" data-table="as_retur_suppliers" data-field="x_createdUserID" name="x_createdUserID" id="x_createdUserID" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->createdUserID->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->createdUserID->EditValue ?>"<?php echo $as_retur_suppliers->createdUserID->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->createdUserID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->modifiedDate->Visible) { // modifiedDate ?>
	<div id="r_modifiedDate" class="form-group">
		<label id="elh_as_retur_suppliers_modifiedDate" for="x_modifiedDate" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->modifiedDate->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->modifiedDate->CellAttributes() ?>>
<span id="el_as_retur_suppliers_modifiedDate">
<input type="text" data-table="as_retur_suppliers" data-field="x_modifiedDate" name="x_modifiedDate" id="x_modifiedDate" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->modifiedDate->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->modifiedDate->EditValue ?>"<?php echo $as_retur_suppliers->modifiedDate->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->modifiedDate->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($as_retur_suppliers->modifiedUserID->Visible) { // modifiedUserID ?>
	<div id="r_modifiedUserID" class="form-group">
		<label id="elh_as_retur_suppliers_modifiedUserID" for="x_modifiedUserID" class="col-sm-2 control-label ewLabel"><?php echo $as_retur_suppliers->modifiedUserID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $as_retur_suppliers->modifiedUserID->CellAttributes() ?>>
<span id="el_as_retur_suppliers_modifiedUserID">
<input type="text" data-table="as_retur_suppliers" data-field="x_modifiedUserID" name="x_modifiedUserID" id="x_modifiedUserID" size="30" placeholder="<?php echo ew_HtmlEncode($as_retur_suppliers->modifiedUserID->getPlaceHolder()) ?>" value="<?php echo $as_retur_suppliers->modifiedUserID->EditValue ?>"<?php echo $as_retur_suppliers->modifiedUserID->EditAttributes() ?>>
</span>
<?php echo $as_retur_suppliers->modifiedUserID->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$as_retur_suppliers_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $as_retur_suppliers_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php if (!isset($as_retur_suppliers_edit->Pager)) $as_retur_suppliers_edit->Pager = new cPrevNextPager($as_retur_suppliers_edit->StartRec, $as_retur_suppliers_edit->DisplayRecs, $as_retur_suppliers_edit->TotalRecs) ?>
<?php if ($as_retur_suppliers_edit->Pager->RecordCount > 0 && $as_retur_suppliers_edit->Pager->Visible) { ?>
...
<script type="text/javascript">
fas_retur_suppliersedit.Init();
</script>
<?php
$as_retur_suppliers_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$as_retur_suppliers_edit->Page_Terminate();
?>
