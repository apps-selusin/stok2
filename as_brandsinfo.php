...
//
// Table class for as_brands
//
class cas_brands extends cTable {
	var $brandID;
	var $brandName;
	var $status;
	var $createdDate;
	...
	function __construct() {
		...
		$this->TableVar = 'as_brands';
		$this->TableName = 'as_brands';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`as_brands`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// brandID
		$this->brandID = new cField('as_brands', 'as_brands', 'x_brandID', 'brandID', '`brandID`', '`brandID`', 3, -1, FALSE, '`brandID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// brandName
		$this->brandName = new cField('as_brands', 'as_brands', 'x_brandName', 'brandName', '`brandName`', '`brandName`', 200, -1, FALSE, '`brandName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->brandName->Sortable = TRUE; // Allow sort
		$this->fields['brandName'] = &$this->brandName;

		// status
		$this->status = new cField('as_brands', 'as_brands', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->status->Sortable = TRUE; // Allow sort
		$this->fields['status'] = &$this->status;

		// createdDate
		$this->createdDate = new cField('as_brands', 'as_brands', 'x_createdDate', 'createdDate', '`createdDate`', ew_CastDateFieldForLike('`createdDate`', 0, "DB"), 135, 0, FALSE, '`createdDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// createdUserID
		$this->createdUserID = new cField('as_brands', 'as_brands', 'x_createdUserID', 'createdUserID', '`createdUserID`', '`createdUserID`', 3, -1, FALSE, '`createdUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedDate
		$this->modifiedDate = new cField('as_brands', 'as_brands', 'x_modifiedDate', 'modifiedDate', '`modifiedDate`', ew_CastDateFieldForLike('`modifiedDate`', 0, "DB"), 135, 0, FALSE, '`modifiedDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedUserID
		$this->modifiedUserID = new cField('as_brands', 'as_brands', 'x_modifiedUserID', 'modifiedUserID', '`modifiedUserID`', '`modifiedUserID`', 3, -1, FALSE, '`modifiedUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`as_brands`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->brandID->setDbValue($conn->Insert_ID());
			$rs['brandID'] = $this->brandID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('brandID', $rs))
				ew_AddFilter($where, ew_QuotedName('brandID', $this->DBID) . '=' . ew_QuotedValue($rs['brandID'], $this->brandID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->brandID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@brandID@", ew_AdjustSql($this->brandID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("as_brandsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("as_brandsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "as_brandsadd.php?" . $this->UrlParm($parm);
		else
			$url = "as_brandsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("as_brandsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("as_brandsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("as_brandsdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "brandID:" . ew_VarToJson($this->brandID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->brandID->CurrentValue)) {
			$sUrl .= "brandID=" . urlencode($this->brandID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["brandID"]))
				$arKeys[] = ew_StripSlashes($_POST["brandID"]);
			elseif (isset($_GET["brandID"]))
				$arKeys[] = ew_StripSlashes($_GET["brandID"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->brandName->EditValue = $this->brandName->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->brandName->Exportable) $Doc->ExportCaption($this->brandName);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->brandName->Exportable) $Doc->ExportField($this->brandName);
