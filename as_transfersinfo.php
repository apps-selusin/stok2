...
//
// Table class for as_transfers
//
class cas_transfers extends cTable {
	var $transferID;
	var $transferCode;
	var $transferFaktur;
	var $ref;
	...
	function __construct() {
		...
		$this->TableVar = 'as_transfers';
		$this->TableName = 'as_transfers';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`as_transfers`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// transferID
		$this->transferID = new cField('as_transfers', 'as_transfers', 'x_transferID', 'transferID', '`transferID`', '`transferID`', 3, -1, FALSE, '`transferID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->transferID->Sortable = TRUE; // Allow sort
		$this->transferID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['transferID'] = &$this->transferID;

		// transferCode
		$this->transferCode = new cField('as_transfers', 'as_transfers', 'x_transferCode', 'transferCode', '`transferCode`', '`transferCode`', 200, -1, FALSE, '`transferCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->transferCode->Sortable = TRUE; // Allow sort
		$this->fields['transferCode'] = &$this->transferCode;

		// transferFaktur
		$this->transferFaktur = new cField('as_transfers', 'as_transfers', 'x_transferFaktur', 'transferFaktur', '`transferFaktur`', '`transferFaktur`', 200, -1, FALSE, '`transferFaktur`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->transferFaktur->Sortable = TRUE; // Allow sort
		$this->fields['transferFaktur'] = &$this->transferFaktur;

		// ref
		$this->ref = new cField('as_transfers', 'as_transfers', 'x_ref', 'ref', '`ref`', '`ref`', 200, -1, FALSE, '`ref`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ref->Sortable = TRUE; // Allow sort
		$this->fields['ref'] = &$this->ref;

		// trxDate
		$this->trxDate = new cField('as_transfers', 'as_transfers', 'x_trxDate', 'trxDate', '`trxDate`', ew_CastDateFieldForLike('`trxDate`', 0, "DB"), 133, 0, FALSE, '`trxDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->trxDate->Sortable = TRUE; // Allow sort
		$this->trxDate->FldDefaultErrMsg = str_replace("%s", $GLOBALS["EW_DATE_FORMAT"], $Language->Phrase("IncorrectDate"));
		$this->fields['trxDate'] = &$this->trxDate;

		// note
		$this->note = new cField('as_transfers', 'as_transfers', 'x_note', 'note', '`note`', '`note`', 201, -1, FALSE, '`note`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->note->Sortable = TRUE; // Allow sort
		$this->fields['note'] = &$this->note;

		// transferFrom
		$this->transferFrom = new cField('as_transfers', 'as_transfers', 'x_transferFrom', 'transferFrom', '`transferFrom`', '`transferFrom`', 3, -1, FALSE, '`transferFrom`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->transferFrom->Sortable = TRUE; // Allow sort
		$this->transferFrom->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['transferFrom'] = &$this->transferFrom;

		// transferTo
		$this->transferTo = new cField('as_transfers', 'as_transfers', 'x_transferTo', 'transferTo', '`transferTo`', '`transferTo`', 3, -1, FALSE, '`transferTo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->transferTo->Sortable = TRUE; // Allow sort
		$this->transferTo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['transferTo'] = &$this->transferTo;

		// createdDate
		$this->createdDate = new cField('as_transfers', 'as_transfers', 'x_createdDate', 'createdDate', '`createdDate`', ew_CastDateFieldForLike('`createdDate`', 0, "DB"), 135, 0, FALSE, '`createdDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// createdUserID
		$this->createdUserID = new cField('as_transfers', 'as_transfers', 'x_createdUserID', 'createdUserID', '`createdUserID`', '`createdUserID`', 3, -1, FALSE, '`createdUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedDate
		$this->modifiedDate = new cField('as_transfers', 'as_transfers', 'x_modifiedDate', 'modifiedDate', '`modifiedDate`', ew_CastDateFieldForLike('`modifiedDate`', 0, "DB"), 135, 0, FALSE, '`modifiedDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedUserID
		$this->modifiedUserID = new cField('as_transfers', 'as_transfers', 'x_modifiedUserID', 'modifiedUserID', '`modifiedUserID`', '`modifiedUserID`', 3, -1, FALSE, '`modifiedUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`as_transfers`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('transferID', $rs))
				ew_AddFilter($where, ew_QuotedName('transferID', $this->DBID) . '=' . ew_QuotedValue($rs['transferID'], $this->transferID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->transferID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@transferID@", ew_AdjustSql($this->transferID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("as_transfersview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("as_transfersview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "as_transfersadd.php?" . $this->UrlParm($parm);
		else
			$url = "as_transfersadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("as_transfersedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("as_transfersadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("as_transfersdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "transferID:" . ew_VarToJson($this->transferID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->transferID->CurrentValue)) {
			$sUrl .= "transferID=" . urlencode($this->transferID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["transferID"]))
				$arKeys[] = ew_StripSlashes($_POST["transferID"]);
			elseif (isset($_GET["transferID"]))
				$arKeys[] = ew_StripSlashes($_GET["transferID"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->transferCode->EditValue = $this->transferCode->CurrentValue;
		...
		$this->transferFaktur->EditValue = $this->transferFaktur->CurrentValue;
		...
		$this->trxDate->EditValue = ew_FormatDateTime($this->trxDate->CurrentValue, 8);
		...
		$this->transferFrom->EditValue = $this->transferFrom->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->transferID->Exportable) $Doc->ExportCaption($this->transferID);
					if ($this->transferCode->Exportable) $Doc->ExportCaption($this->transferCode);
					if ($this->transferFaktur->Exportable) $Doc->ExportCaption($this->transferFaktur);
					if ($this->ref->Exportable) $Doc->ExportCaption($this->ref);
					if ($this->trxDate->Exportable) $Doc->ExportCaption($this->trxDate);
					if ($this->note->Exportable) $Doc->ExportCaption($this->note);
					if ($this->transferFrom->Exportable) $Doc->ExportCaption($this->transferFrom);
					if ($this->transferTo->Exportable) $Doc->ExportCaption($this->transferTo);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->transferID->Exportable) $Doc->ExportField($this->transferID);
						if ($this->transferCode->Exportable) $Doc->ExportField($this->transferCode);
						if ($this->transferFaktur->Exportable) $Doc->ExportField($this->transferFaktur);
						if ($this->ref->Exportable) $Doc->ExportField($this->ref);
						if ($this->trxDate->Exportable) $Doc->ExportField($this->trxDate);
						if ($this->note->Exportable) $Doc->ExportField($this->note);
						if ($this->transferFrom->Exportable) $Doc->ExportField($this->transferFrom);
						if ($this->transferTo->Exportable) $Doc->ExportField($this->transferTo);
