...
//
// Table class for as_receivables
//
class cas_receivables extends cTable {
	var $receiveID;
	var $receiveNo;
	var $invoiceID;
	var $invoiceNo;
	...
	function __construct() {
		...
		$this->TableVar = 'as_receivables';
		$this->TableName = 'as_receivables';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`as_receivables`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// receiveID
		$this->receiveID = new cField('as_receivables', 'as_receivables', 'x_receiveID', 'receiveID', '`receiveID`', '`receiveID`', 3, -1, FALSE, '`receiveID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->receiveID->Sortable = TRUE; // Allow sort
		$this->receiveID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['receiveID'] = &$this->receiveID;

		// receiveNo
		$this->receiveNo = new cField('as_receivables', 'as_receivables', 'x_receiveNo', 'receiveNo', '`receiveNo`', '`receiveNo`', 200, -1, FALSE, '`receiveNo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->receiveNo->Sortable = TRUE; // Allow sort
		$this->fields['receiveNo'] = &$this->receiveNo;

		// invoiceID
		$this->invoiceID = new cField('as_receivables', 'as_receivables', 'x_invoiceID', 'invoiceID', '`invoiceID`', '`invoiceID`', 3, -1, FALSE, '`invoiceID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// invoiceNo
		$this->invoiceNo = new cField('as_receivables', 'as_receivables', 'x_invoiceNo', 'invoiceNo', '`invoiceNo`', '`invoiceNo`', 200, -1, FALSE, '`invoiceNo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->invoiceNo->Sortable = TRUE; // Allow sort
		$this->fields['invoiceNo'] = &$this->invoiceNo;

		// customerID
		$this->customerID = new cField('as_receivables', 'as_receivables', 'x_customerID', 'customerID', '`customerID`', '`customerID`', 3, -1, FALSE, '`customerID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// customerName
		$this->customerName = new cField('as_receivables', 'as_receivables', 'x_customerName', 'customerName', '`customerName`', '`customerName`', 200, -1, FALSE, '`customerName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->customerName->Sortable = TRUE; // Allow sort
		$this->fields['customerName'] = &$this->customerName;

		// customerAddress
		$this->customerAddress = new cField('as_receivables', 'as_receivables', 'x_customerAddress', 'customerAddress', '`customerAddress`', '`customerAddress`', 201, -1, FALSE, '`customerAddress`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->customerAddress->Sortable = TRUE; // Allow sort
		$this->fields['customerAddress'] = &$this->customerAddress;

		// receiveTotal
		$this->receiveTotal = new cField('as_receivables', 'as_receivables', 'x_receiveTotal', 'receiveTotal', '`receiveTotal`', '`receiveTotal`', 5, -1, FALSE, '`receiveTotal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->receiveTotal->Sortable = TRUE; // Allow sort
		$this->receiveTotal->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['receiveTotal'] = &$this->receiveTotal;

		// incomingTotal
		$this->incomingTotal = new cField('as_receivables', 'as_receivables', 'x_incomingTotal', 'incomingTotal', '`incomingTotal`', '`incomingTotal`', 5, -1, FALSE, '`incomingTotal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// reductionTotal
		$this->reductionTotal = new cField('as_receivables', 'as_receivables', 'x_reductionTotal', 'reductionTotal', '`reductionTotal`', '`reductionTotal`', 5, -1, FALSE, '`reductionTotal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// status
		$this->status = new cField('as_receivables', 'as_receivables', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->status->Sortable = TRUE; // Allow sort
		$this->fields['status'] = &$this->status;

		// staffID
		$this->staffID = new cField('as_receivables', 'as_receivables', 'x_staffID', 'staffID', '`staffID`', '`staffID`', 3, -1, FALSE, '`staffID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// staffName
		$this->staffName = new cField('as_receivables', 'as_receivables', 'x_staffName', 'staffName', '`staffName`', '`staffName`', 200, -1, FALSE, '`staffName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->staffName->Sortable = TRUE; // Allow sort
		$this->fields['staffName'] = &$this->staffName;

		// createdDate
		$this->createdDate = new cField('as_receivables', 'as_receivables', 'x_createdDate', 'createdDate', '`createdDate`', ew_CastDateFieldForLike('`createdDate`', 0, "DB"), 135, 0, FALSE, '`createdDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// createdUserID
		$this->createdUserID = new cField('as_receivables', 'as_receivables', 'x_createdUserID', 'createdUserID', '`createdUserID`', '`createdUserID`', 3, -1, FALSE, '`createdUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedDate
		$this->modifiedDate = new cField('as_receivables', 'as_receivables', 'x_modifiedDate', 'modifiedDate', '`modifiedDate`', ew_CastDateFieldForLike('`modifiedDate`', 0, "DB"), 135, 0, FALSE, '`modifiedDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// modifiedUserID
		$this->modifiedUserID = new cField('as_receivables', 'as_receivables', 'x_modifiedUserID', 'modifiedUserID', '`modifiedUserID`', '`modifiedUserID`', 3, -1, FALSE, '`modifiedUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`as_receivables`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->receiveID->setDbValue($conn->Insert_ID());
			$rs['receiveID'] = $this->receiveID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('receiveID', $rs))
				ew_AddFilter($where, ew_QuotedName('receiveID', $this->DBID) . '=' . ew_QuotedValue($rs['receiveID'], $this->receiveID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`receiveID` = @receiveID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->receiveID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@receiveID@", ew_AdjustSql($this->receiveID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("as_receivablesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("as_receivablesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "as_receivablesadd.php?" . $this->UrlParm($parm);
		else
			$url = "as_receivablesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("as_receivablesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("as_receivablesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("as_receivablesdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "receiveID:" . ew_VarToJson($this->receiveID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->receiveID->CurrentValue)) {
			$sUrl .= "receiveID=" . urlencode($this->receiveID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["receiveID"]))
				$arKeys[] = ew_StripSlashes($_POST["receiveID"]);
			elseif (isset($_GET["receiveID"]))
				$arKeys[] = ew_StripSlashes($_GET["receiveID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->receiveID->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->receiveNo->EditValue = $this->receiveNo->CurrentValue;
		...
		$this->receiveTotal->EditValue = $this->receiveTotal->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->receiveID->Exportable) $Doc->ExportCaption($this->receiveID);
					if ($this->receiveNo->Exportable) $Doc->ExportCaption($this->receiveNo);
					...
					if ($this->receiveTotal->Exportable) $Doc->ExportCaption($this->receiveTotal);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->receiveID->Exportable) $Doc->ExportField($this->receiveID);
						if ($this->receiveNo->Exportable) $Doc->ExportField($this->receiveNo);
						...
						if ($this->receiveTotal->Exportable) $Doc->ExportField($this->receiveTotal);
